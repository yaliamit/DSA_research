...
def fastrcnn_loss(class_logits, box_regression, occlusion_prediction, labels, regression_targets):
    ...
    #regression_targets = regression_targets[:, 0:4]
    occlusion_targets = labels[:, 1]
    labels = labels[:, 0]
    # occlusion_prediction = occlusion_prediction.view(-1,1)
    # occlusion_prediction = torch.cat((1-occlusion_prediction, occlusion_prediction), 1)

    #print(class_logits.shape, labels.shape, occlusion_prediction.shape, occlusion_targets.shape)
    classification_loss = F.cross_entropy(class_logits, labels)
    occlusion_loss = F.cross_entropy(occlusion_prediction, occlusion_targets)
    ...
    #print(classification_loss, box_loss, occlusion_loss)

    return classification_loss+occlusion_loss, box_loss
...
class RoIHeads(nn.Module):
    ...
    def postprocess_detections(self,
        ...
        pred_occlusion_scores = F.softmax(occlusion_prediction, -1)

        pred_boxes_list = pred_boxes.split(boxes_per_image, 0)
        pred_scores_list = pred_scores.split(boxes_per_image, 0)
        pred_occlusion_scores_list = pred_occlusion_scores.split(boxes_per_image, 0)

        all_boxes = []
        all_scores = []
        all_labels = []
        all_occlusion_scores = []
        for boxes, scores, occlusion_scores, image_shape in zip(pred_boxes_list, pred_scores_list, pred_occlusion_scores_list, image_shapes):
            ...
            occlusion_scores = occlusion_scores.reshape(-1)
            #print('boxes', boxes[0, :])

            # remove low scoring boxes
            inds = torch.where(scores > self.score_thresh)[0]
            #print(boxes.shape, scores.shape, labels.shape, occlusion_scores.shape, inds)
            #print(labels, occlusion_scores)
            boxes, scores, labels, occlusion_scores = boxes[inds], scores[inds], labels[inds], occlusion_scores[inds]
            #print(boxes.shape, scores.shape, labels.shape, occlusion_scores.shape)

            # remove empty boxes
            #print(labels, boxes)
            keep = box_ops.remove_small_boxes(boxes, min_size=1e-2)
            boxes, scores, labels, occlusion_scores = boxes[keep], scores[keep], labels[keep], occlusion_scores[keep]
            #print(boxes.shape, scores.shape, labels.shape, occlusion_scores.shape)
            ...
            all_occlusion_scores.append(occlusion_scores)

        return all_boxes, all_scores, all_labels, all_occlusion_scores
    ...
    def forward(self,
        ...
        class_logits, box_regression, occlusion_prediction = self.box_predictor(box_features)
        ...
        if self.training:
            ...
            loss_classifier, loss_box_reg = fastrcnn_loss(
                class_logits, box_regression, occlusion_prediction, labels, regression_targets)
            losses = {
                "loss_classifier": loss_classifier,
                "loss_box_reg": loss_box_reg
            }
        else:
            boxes, scores, labels, occlusion_scores = self.postprocess_detections(class_logits, box_regression, occlusion_prediction, proposals, image_shapes)
                ...
                result.append(
                    {
                        ...
                        "labels": labels[i],
                        "scores": scores[i],
                        "occlusion_scores": occlusion_scores[i],
                    }
                )
